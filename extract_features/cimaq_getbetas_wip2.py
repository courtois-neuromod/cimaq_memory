...
def get_task_Files(tDir):
    ...
    if not os.path.exists(tDir):
        ...
        return
    task_files = glob.glob(os.path.join(tDir,'sub-*events.tsv'))
    return task_files
...
def get_subject_betas(id, confounds, all_events, fmridir, outdir):
    ...
    if len(scanfile) == 0:
        ...
        for i in range (0, numTrials):
            ...
            b_name = os.path.join(sub_outdir, 'betas_sub'+str(id)+'_Trial'+str(tnum)+'_'+tname+'.nii')
...
def extract_betas(taskList, outdir, motiondir, fmridir):
    ...
    None (beta maps are saved directly in outdir)
    """
    b_outdir = os.path.join(outdir, 'Betas')
    if not os.path.exists(b_outdir):
        os.mkdir(b_outdir)

    ev_outdir = os.path.join(outdir, 'Events')
...
def main():
    args =  get_arguments()
    task_files = get_task_Files(args.tdir[0])
