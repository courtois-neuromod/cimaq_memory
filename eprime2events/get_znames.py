...
def get_znames(zfile: Union[str, os.PathLike, ZipFile],
    ...
    Lists the contents of a .zip archive. User can filter what names are returned
    by specifying desired file extensions, what names to include or exclude
    and a minimum file size.
    ...
    of the archive might have been generated and added to its contents during compression.
    These behave as if they were regular archives, but are error prone,
    sometimes unreadable or empty. The default behavior is to exclude such files.
    ...
    Args:
        ...
        exts: str, set of str, Sequence of str or None, optional
            File extensions to look for within the archive.
            Can be a single string, a set or a Sequence.
            Single strings are inserted in a list, other types are converted to a list.
        to_include: str, set of str, Sequence of str  or None, optional
            Patterns or names that should be found in the names of the returned list.
            Can be a single string, a set or a Sequence.
            Single strings are inserted in a list, other types are converted to a list.
        to_exclude: str, set of str, Sequence of str  or None, optional
            Patterns or names that should NOT be found in the names of the returned list.
            Can be a single string, a set or a Sequence.
            Single strings are inserted in a list, other types are converted to a list.
        min_size: int, default 103
            Minimum required archive size for its name to be returned. The default value
            of 103 corresponds to a file containing a single maximum possible lenght
            UTF-8 encoded character. A value of 0 means no minimum size is required.
        default_filtering: bool, default True
            Exclude commonly error-prone files from the returned names. These are
            ['.DS_Store', '_MACOSX', 'textClipping'], which are generally empty.
...
def main():
    FilterType = NewType('FilterType', [str, Sequence])
    ...
    parser.add_argument('--exts', dest='exts', type=FilterType, nargs='?',
                        default=None, required=False)
    parser.add_argument('-i', '--include', dest='to_include', type=FilterType,
                        nargs='?', default=None, required=False)
    parser.add_argument('-e', '--exclude', dest='to_exclude', type=FilterType,
                        nargs='?', default=badnames, required=False)
    parser.add_argument('-m', '--min-size', dest='min_size', type=int, nargs='?',
                        default=103, required=False)
    parser.add_argument('--no-filter', dest='default_filtering', action='store_false')                        
    parser.add_argument('-k', '--keep-open', dest='keep_open', action='store_true')
    ...
    get_znames(args.zfile[0], args.exts, args.to_include, args.to_exclude,
               args.min_size, args.default_filtering, args.keep_open, args.prnt)
